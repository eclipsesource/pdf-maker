...
function layoutParagraph(content: Paragraph, pos: Pos, size: Size, fonts: Font[]): Frame {
  const maxWidth = size.width;
  const maxHeight = size.height;
  const box = { x: pos.x, y: pos.y, width: maxWidth, height: maxHeight };
  const { text, ...attrs } = content;
  const segments = extractTextSegments(text, attrs, fonts);
  return layoutRow(segments, box);
}
